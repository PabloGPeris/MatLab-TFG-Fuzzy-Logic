...
% Cálculo de tamaños
sizephi1 = 1; %variable auxiliar
for i = 1:length(fp)
    sizephi1 = sizephi1 * length(fp{i});
end
...
heightphi = heightphi + sizephi;

% Inicialización de variables
phi = zeros(heightphi, sizephi); % matriz de entradas y salidas (llamada X, creo, en el Takagi Sugeno)
yr = zeros(heightphi, 1); % salida p resultado (no es anidar todas las salidas, solo a partir del orden del sistema)
inputs = cell(1,length(varargin));
...
yr(k1:end,:) = kron(ones(sizephi1,1), linearparam)*gamma; % añade 0's a las salidas yr (debería poner lo de que sean los parámetros lineales, lo sé)
phi(k1:end,:) = eye(sizephi)*gamma; 
