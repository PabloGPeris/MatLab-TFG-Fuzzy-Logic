function [Phi, Yr] = FuzzyPhiMatrixPolar(Y, U, format, Vb, FSetArg, FSetMod)
%PHIMATRIXMISO(Y, [U(1) ... U(N)], format, Vb, FSetArg, FSetMod)
...
%
%   Vb es el vector de variables borrosas [argumento, módulo].
%
%   FSetArg es el vector de FuzzySetArgumento, de longitud la misma que
%   FSetMod (el FuzzySet del módulo).
%% Inicio
...
if length(FSetArg) ~= FSLength(FSetMod)
    error('FSetArg debe ser un vector de FuzzySet de longitud FS de FSetMod');
end
...
for i = 1:num_filas % i = fila
    ...
    w = Fuzzification_polar(Vb(orden + i - 1,:), FSetArg, FSetMod);
    Phi(i,:) = kron(w,row);
end
